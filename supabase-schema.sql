...
-- TRUCK EXPENSES (Despesas do Caminhão)
...
create table if not exists public.closings (
    ...
    "totalLitros" numeric,
    "totalKm" numeric,
    "totalKmFretes" numeric,
    "mediaConsumo" numeric,
    saldo numeric,
    "qtdAbastecimentos" integer,
    "qtdFretes" integer,
    "qtdMultas" integer,
    "qtdDespesas" integer,
    "fuelings" jsonb,
    "fuelingsForMedia" jsonb,
    "geradoEm" text,
...
-- MAINTENANCE PLANS (Manutenção Preventiva)
create table if not exists public.maintenance_plans (
    id bigint generated by default as identity primary key,
    user_id uuid references auth.users not null default auth.uid(),
    "truckId" bigint not null references public.trucks(id) on delete cascade,
    item text not null,
...
create policy "Users can CRUD their own data" on public.maintenance_plans for all using (auth.uid() = user_id);
...
create table if not exists public.tires (
    ...
    fogo text,
    marca text,
    "kmInstalacao" integer,
    status text default 'Novo',
    created_at timestamp with time zone default timezone('utc'::text, now()) not null
);
alter table public.tires enable row level security;
create policy "Users can CRUD their own data" on public.tires for all using (auth.uid() = user_id);

-- TIRES HISTORY (Cemitério de Pneus)
create table if not exists public.tires_history (
    id bigint generated by default as identity primary key,
    user_id uuid references auth.users not null default auth.uid(),
    "truckId" bigint not null references public.trucks(id) on delete cascade,
    "posicaoOriginal" text not null,
    "eixoOriginal" integer not null,
    fogo text,
    marca text,
    "kmInstalacao" integer,
    "kmRetirada" integer,
    "kmRodados" integer,
    "statusNaRetirada" text,
    motivo text,
    created_at timestamp with time zone default timezone('utc'::text, now()) not null
);
alter table public.tires_history enable row level security;
create policy "Users can CRUD their own data" on public.tires_history for all using (auth.uid() = user_id);
